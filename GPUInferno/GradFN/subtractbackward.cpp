#include "subtractbackward.h"


namespace Inferno {


	SubtractBackward::SubtractBackward(const Tensor& A, const Tensor& B) : m_A(A), m_B(B) {


	}

	void SubtractBackward::backward() {
	...
	void SubtractBackward::release() {
		// drop references so graph can free
		m_A = Tensor{};
		m_B = Tensor{};
	}

	void SubtractBackward::get_inputs(std::vector<Tensor>& out) const {
		out.push_back(m_A);
		out.push_back(m_B);
	}
