...
namespace Inferno {
    ...
    __global__ void add_kernel_broadcast(const AT* aptr,const BT* bptr,RT* outptr,size_t out_numel,int out_rank,
    ...
    template<typename AT, typename BT, typename RT>
    void cuda_add(const AT* aptr,const BT* bptr,RT* outptr,const std::vector<size_t>& ashape,const std::vector<size_t>& bshape,
        ...
        if (out_rank > MAX_DIMS) {
            Logger::Append(Logger::LogLevel::LOGLEVEL_ERROR,
                "cuda_add: out rank exceeds MAX_DIMS");
            exit(1);
        }
        ...
        check_cuda(cudaMemcpy(d_a_padded, a_padded.data(), out_rank * sizeof(size_t), cudaMemcpyHostToDevice), "cuda_add cudaMemcpy d_a_padded failed");
        check_cuda(cudaMemcpy(d_b_padded, b_padded.data(), out_rank * sizeof(size_t), cudaMemcpyHostToDevice), "cuda_add cudaMemcpy d_b_padded failed");
        check_cuda(cudaMemcpy(d_out_shape, out_shape.data(), out_rank * sizeof(size_t), cudaMemcpyHostToDevice), "cuda_add cudaMemcpy d_out_shape failed");
        check_cuda(cudaMemcpy(d_a_strides, a_strides.data(), out_rank * sizeof(size_t), cudaMemcpyHostToDevice), "cuda_add cudaMemcpy d_a_strides failed");
        check_cuda(cudaMemcpy(d_b_strides, b_strides.data(), out_rank * sizeof(size_t), cudaMemcpyHostToDevice), "cuda_add cudaMemcpy d_b_strides failed");

        constexpr int threads = 256;
        int blocks = static_cast<int>((out_numel + threads - 1) / threads);

        add_kernel_broadcast<AT, BT, RT> << <blocks, threads >> > (
            aptr,
            bptr,
            outptr,
            out_numel,
        ...
        check_cuda(cudaDeviceSynchronize(), "cuda_add kernel execution failed");
    ...
    template void cuda_add<int, int, int>(
    ...
    template void cuda_add<float, float, float>(
    ...
    template void cuda_add<double, double, double>(
    ...
    template void cuda_add<int, float, float>(
    ...
    template void cuda_add<float, int, float>(
    ...
    template void cuda_add<int, double, double>(
    ...
    template void cuda_add<double, int, double>(
    ...
    template void cuda_add<float, double, double>(
    ...
    template void cuda_add<double, float, double>(
