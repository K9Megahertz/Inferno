...
namespace Inferno {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    //  Function divide_kernel_broadcast
    //
    //
    //
    //
    ...
    void cuda_divide(const AT* aptr,
        ...
        check_cuda(cudaMalloc(&d_a_padded, out_rank * sizeof(size_t)), "cuda_divide cudaMalloc d_a_padded failed");
        check_cuda(cudaMalloc(&d_b_padded, out_rank * sizeof(size_t)), "cuda_divide cudaMalloc d_b_padded failed");
        check_cuda(cudaMalloc(&d_out_shape, out_rank * sizeof(size_t)), "cuda_divide cudaMalloc d_out_shape failed");
        ...
        divide_kernel_broadcast<AT, BT, RT> << <blocks, threads >> > (
            aptr,
            bptr,
            outptr,
            out_numel,
        ...
        check_cuda(cudaGetLastError(), "cuda_divide kernel launch failed");
        check_cuda(cudaDeviceSynchronize(), "cuda_divide kernel execution failed");

        check_cuda(cudaFree(d_a_padded), "cuda_divide cudaFree d_a_padded failed");
        check_cuda(cudaFree(d_b_padded), "cuda_divide cudaFree d_b_padded failed");
        check_cuda(cudaFree(d_out_shape), "cuda_divide cudaFree d_out_shape failed");
        check_cuda(cudaFree(d_a_strides), "cuda_divide cudaFree d_a_strides failed");
        check_cuda(cudaFree(d_b_strides), "cuda_divide cudaFree d_b_strides failed");
    }
    ...
    // explicit instantiations
    template void cuda_divide<int, int, float>(
    ...
    template void cuda_divide<float, float, float>(
    ...
    template void cuda_divide<double, double, double>(
    ...
    template void cuda_divide<int, float, float>(
    ...
    template void cuda_divide<float, int, float>(
    ...
    template void cuda_divide<int, double, double>(
    ...
    template void cuda_divide<double, int, double>(
    ...
    template void cuda_divide<float, double, double>(
    ...
    template void cuda_divide<double, float, double>(
