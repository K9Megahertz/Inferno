...
namespace Inferno {
	...
	Tensor sum_to_shape(const Tensor& src, const std::vector<size_t>& target_shape) {
		...
		return dispatchAny(src.dtype(), [&](auto TA) {
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//  Function scatter_add_embedding
	//
	//
	//
	//
	...
	Tensor scatter_add_embedding(const Tensor& embeddings, const Tensor& token_ids, const Tensor& g_out) {
		...
		return dispatchInt(token_ids.dtype(), [&](auto TagA) {
			using AT = typename decltype(TagA)::type;  //type for tokens			

			return dispatchFloat(embeddings.dtype(), [&](auto TagB) {
	...
	Tensor scatter_add_slice(Tensor& A, const Tensor& g_out, int axis, size_t start, size_t step) {
		...
		return dispatchAny(A.dtype(), [&](auto TagA) {
